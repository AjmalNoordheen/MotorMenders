...
function Popup({ fun, sendMessage }) {
  ...
  useEffect(() => {
    AdminAxios.get("/listTypes")
      .then((res) => {
        console.log('pppppp');
        if (res.status) {
          console.log(res.status,'p0000000000000000000000000');
  ...
  const updateValue = async () => {
    try {
      ...
      const time = starTime + " am -" + " to - " + " pm " + endTime;
    ...
    <div className="min-h-screen flex overflow-scroll items-center justify-center bg-gray-100">
      <div className="w-9/12 max-w-xl p-4 bg-white rounded-md shadow-xl my-10">
        <form className="py-4">
          ...
          {/* <button
            type="button"
            className="w-full py-2 text-white bg-blue-600 rounded-md hover:bg-blue-700 focus:outline-none focus:ring mt-4"
            onClick={updateValue}
          >
            Continue
          </button> */}
