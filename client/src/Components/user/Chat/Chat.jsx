...
function Chat({fun,id}) {
    ...
    useEffect(()=>{
         userAxios.get(`/loadChat?email=${email}&proId=${id}&type=${userType}&id=${userData._id}`).then((res)=>{
            if(res){
...
//       }, [UserData]);
      ...
      const sendMessage=async()=>{
         try {
            ...
            if(message.length>0){
             const res =await userAxios.post('/addMessage',{message,chatId,UserDataEmail,userType})
            if(res){
                let newMessage = {text:message,senderType:userType,senderId:userData._id}
                setMessage('')
                setMessages(res.data.messages)
                soket.emit('newMessage',newMessage,chatId)
            }
            }
        } catch (error) {
            console.log(error)
     ...
     <div className="flex h-screen full antialiased justify-center items-center text-gray-800">
              ...
              <div className="flex flex-col flex-auto flex-shrink-0 rounded-2xl bg-gray-100 h-full p-4">
                <div className="flex flex-col h-full overflow-x-auto mb-4" ref={messageHolder}>
                  <span className='cursor-pointer fixed' onClick={()=>fun(1)}><i class="fa-solid fa-arrow-left"></i></span>

                  <div className="flex flex-col h-full " >
                    {messages.length > 0 ? messages.map((message) => (
                      <div key={message._id} className="grid grid-cols-12 gap-y-2">
                        {(message?.senderId) == userData._id ? <div className="col-start-7 col-end-13 p-3 rounded-lg">
                          <div className="flex items-center justify-start flex-row-reverse">
                            ...
                            <div className="relative mr-3 w-full text-sm bg-indigo-100 py-2 px-4 shadow rounded-xl">
                              <div className='break-words'>{message?message.text:''}</div>
