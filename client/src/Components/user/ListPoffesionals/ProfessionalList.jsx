...
const filteredPros = pros.filter((pro) => {
  ...
  return (
    ...
    {show != 1?<Chat fun={setShow} id={show}/>:(<>
      <div className="w-full h-full bg-[#1e51db] ">
    <NavBar data={1}/>
    </div>
      <div className="w-full h-full  bg-slate-200 ">
        ...
        <div className="rounded">
          <input type="search"  value={searchInput} onChange={(e)=>SetSearchInput(e.target.value)}  className="mt-[7rem] px-3 rounded" placeholder="search your location" />
        </div>
        </div>
        <div className="w-full h-fit sm:h-screen flex flex-col sm:flex-row justify-evenly">
          ...
          {/* Card Container */}
          <div className="w-full sm:w-9/12 h-fit rounded-lg border border-slate-300  overflow-scroll p-3 pb-12  grid grid-cols-1 sm:grid-cols-12 gap-x-3 bg-white">
            {/* cards */}
            
            {filteredPros.filter((data)=>data.location.toLowerCase().includes(searchInput.toLowerCase())).map(items=>
              <>
              ...
              <h2 className="font-josefin-sans text-base  sm:text-sm text-black">{items.location}</h2>
          ...
          <option key={item._id} value={item.name}>
            {item.name}
              ...
              <div className="flex justify-around">
                ...
                <button onClick={()=>setShow(items._id)} className="text-white bg-[#552a9e] p-1.5 rounded  hover:bg-[#9661f1] hover:text-black"><i className="fa-solid fa-message "></i> Chat</button>
              </div>
              </div>
              </>)}
     ...
     <div className="bg-[url('/footer/carban.jpg')] bg-contain bg-repeat-round h-[20rem] w-[64rem]">
  {/* Your content */}
</div>
     </div>
